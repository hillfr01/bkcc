...
namespace DeveloperSample.Syncing
{
    public class SyncTest
    {
        ...
        [Fact]
        public void CanInitializeCollectionUsingParallel2()
        {
            var debug = new SyncDebug();
            var items = new List<string>();
            ...
            var resultParallel = debug.InitNonConncurrentListParallel(items);
            watch.Stop();
            System.Diagnostics.Debug.WriteLine($"Took {watch.ElapsedMilliseconds} ms for parallel.ForEach().");
            Assert.Equal(items.Count, resultParallel.Count);

            var watch2 = Stopwatch.StartNew();
            var resultSequential = debug.InitNonConncurrentListParallelNo(items);
            watch2.Stop();
            System.Diagnostics.Debug.WriteLine($"Took {watch2.ElapsedMilliseconds} ms for non parallel foreach.");
            Assert.Equal(items.Count, resultSequential.Count);
        }
